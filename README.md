...
```
## kurona
Тут без райтапа, только скажу, что там заложена уязвимость SQL-инъекции (мб потом залью, а так это таска для желающих проверить себя)
# afterword
Делая эти задачи, я пытался показать более-менее реальные кейсы приложений, но по факту хоть как-то близок реальности только `kunigami`, поэтому в описаниях не было подсказок

Задачи были рассчитаны для ребят разных курсов, чтобы менее опытным не было слишком тяжело, а опытным - не так скучно. Так что никогда не отчаивайтесь, когда какой-либо таск не получается решить - это ваш шанс узнать что-то новое
